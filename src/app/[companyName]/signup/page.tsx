    ...
    const checkCompany = async () => {
      ...
      try {
        const res = await fetch(
          `/api/company/check?companyName=${encodeURIComponent(companyName)}`
        );
        if (!res.ok) {
      ...
      <Card className="w-full max-w-sm p-4">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <Label
              htmlFor="number"
              color={error.number ? "failure" : undefined}
            >
              Number
            </Label>
            <PhoneInput
          ...
          <Button
            ...
            disabled={loading || !companyExists || checkingCompany}
          >
            {loading ? (
              <>
